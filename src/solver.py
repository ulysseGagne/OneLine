...
class LongestPathSolver:
    ...
    def backtracking_dfs(self, start, deadline):
        adj = self.adj
        end_set = self.end_nodes  # None or set of valid end nodes
    ...
    def solve(self, time_budget=300):
        ...
        # If we have time left and start is constrained, also try unconstrained
        # to seed the search
        if self.start_nodes is not None:
        ...
        while time.time() < phase2_end:
            start = start_candidates[self.rng.integers(len(start_candidates))]
            path, dist = self.warnsdorff_walk(start, randomize=True)
        ...
        if self.start_nodes is not None:
            # Only add high-degree nodes that are in the start set
        ...
        remaining_time = phase3_end - time.time()
        per_start = remaining_time / max(len(starts), 1)

        for i, start in enumerate(starts):
            if time.time() > phase3_end:
                break
            node_deadline = min(time.time() + per_start, phase3_end)
            path, dist = self.backtracking_dfs(start, node_deadline)
